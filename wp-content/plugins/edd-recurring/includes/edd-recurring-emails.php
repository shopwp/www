...
class EDD_Recurring_Emails {
	...
	public function init() {
		...
		if ( edd_get_option( 'enable_subscription_cancelled_admin_email' ) ) {
			add_action( 'edd_subscription_cancelled', array( $this, 'send_subscription_cancelled_admin' ), 10, 3 );
		}
	}
	...
	public function send_subscription_cancelled_admin( $subscription_id, EDD_Subscription $subscription ) {
		...
		$message  = $this->filter_reminder_template_tags( $message, $subscription_id );
		$message = str_replace( '{subscription_link}', admin_url( 'edit.php?post_type=download&page=edd-subscriptions&id=' . $subscription_id ), $message );
