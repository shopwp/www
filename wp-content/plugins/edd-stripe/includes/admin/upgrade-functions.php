...
function edd_stripe_upgrade_notices() {
	...
	if ( ! edd_has_upgrade_completed( 'stripe_customer_id_migration' ) ) {
		...
		printf(
			...
			'<br /><br />' .
			__( '<strong>Advanced User?</strong><br />This upgrade can also be run via WPCLI with the following command:<br /><code>wp edd-stripe migrate_customer_ids</code>', 'edds' ) .
			'</p>' .
			'</div>',
...
function edd_stripe_customer_id_migration() {
	...
	if ( ! current_user_can( 'manage_shop_settings' ) ) {
		wp_die( __( 'You do not have permission to do shop upgrades', 'edds' ), __( 'Error', 'edds' ), array( 'response' => 403 ) );
	}
