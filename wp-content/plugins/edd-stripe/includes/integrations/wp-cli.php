...
class EDD_Stripe_CLI extends EDD_CLI {
	...
	public function migrate_customer_ids( $args, $assoc_args ) {
		...
		if ( ! $force && $upgrade_completed ) {
			WP_CLI::error( __( 'The Stripe customer ID migration has already been run. To do this anyway, use the --force argument.', 'edds' ) );
		}
		...
		if ( ! empty( $total ) ) {
			...
			$progress->finish();
			WP_CLI::line( __( 'Migration complete.', 'edds' ) );
		} else {
			WP_CLI::line( __( 'No user records were found that needed to be migrated.', 'edds' ) );
		}
