...
if( !class_exists( 'Oauth2Proxy' ) ) {
	...
	class Oauth2Proxy implements IOauth2Proxy
	{
		...
		public function authorize()
		{
			...
			if(isset($_SESSION['vkPhpSdk_user_catch']))
			{
				$this->_authJson = $_SESSION['vkPhpSdk_user_catch'];
				$result = true;
			}
			else
			{
				...
				{
					$_SESSION['vkPhpSdkstate'] = md5(rand(10,100)); // CSRF protection
					...
					$this->_dialogUrl .= '&state=' . $_SESSION['vkPhpSdkstate'];
	
					//echo("<script>top.location.href='" . $this->_dialogUrl . "'</script>");			
					return $this->_dialogUrl;	
				}
				elseif( isset( $_REQUEST['state'] ) && $_REQUEST['state'] === $_SESSION['vkPhpSdkstate'])
				{
					...
					if( !empty( $auth_json ) ) {
						...
						//$_SESSION['vkPhpSdk']['authJson'. $this->_clientId] = $auth_json;
						$_SESSION['vkPhpSdk_user_catch'] = $auth_json;
						$result = true;
					}
					else
						$result = false;
