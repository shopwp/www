...
function ModalContentProfileUpdate({ accountState, accountDispatch }) {
  ...
  async function saveProfile() {
    ...
    if (profileData.confirmNewPassword !== profileData.newPassword) {
      setFormNotice({
        message: 'Your new password does not match. Please try typing the password again.',
        type: 'error',
      });
      setNewPassword('');
  ...
  function onConfirmNewPasswordChange(e) {
    ...
    updateProfileData({
      ...profileData,
      confirmNewPassword: update(profileData.confirmNewPassword, { $set: e.target.value }),
    });
  }
      ...
      <ModalBody>
        ...
        <InputGroup>
          ...
          <Input
            ...
            icon={<IconEye onClick={changeVisibility} visible={passwordVisibility} />}
            disabled={isBusy}
          />
          <Input
            label='New password'
