<section class="component component-affiliate-register">
    <div class="design-widget"></div>
    
    <div class="l-row">

      <div class="l-box-2 affiliate-register-content">
         <h2>Benefits of becoming an Affiliate:</h2>

         <h3>Commissions 💵</h3>
         <p>Earn 20% on each sale of ShopWP Pro (any tier) with your unique referral link. Your tracking cookie will last for 30 days.</p>

         <h3>Quick payouts 🏦</h3>
         <p>Quick and easy payouts directly to your bank account. You'll also have access to a dedicated affiliate dashboard to easy manage your account</p>

         <h3>Support 👨‍💻</h3>
         <p>Priority support via email or live chat in our Slack channel.</p>

         <h3>Branding 🎨</h3>
         <p>Pain-free marketing by providing you with logos and brand materials for your site</p>

      </div>

      <div class="l-box-2 affiliate-register-form">
         <?= do_shortcode('[affiliate_registration]'); ?>
      </div>

   </div>

</section>