...
function ModalContentPaymentUpdate({ accountState, accountDispatch }) {
	...
	const [nameOnCard, setNameOnCard] = useState(() =>
		accountState.subscription.card_info &&
		accountState.subscription.card_info.name
			? accountState.subscription.card_info.name
			: ''
	)
	...
	async function onUpdate() {
		...
		if (error) {
			console.log('ShopWP Error: ', error)
			return
		}

		if (resp) {
			accountDispatch({ type: 'SET_SUBSCRIPTIONS', payload: resp })
		}
		...
		<div>
			...
			<ModalBody>
				<Notice type='info' multiLine={true}>
					...
					{accountState.subscription.card_info &&
						accountState.subscription.card_info.last4}
					) with a new one below. You will not be charged.
				</Notice>
